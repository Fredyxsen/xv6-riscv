...
4. **Modificar `user.h`:**
   * **Declaración para el espacio de usuario:** Para que los programas de usuario puedan usar la syscall, agregamos una declaración en el archivo `user.h`:
     ...
     ```
5. **Modificar `usys.pl`:**
   * **Agregar a la tabla de generación de código:** Este script genera el código necesario para que los programas de usuario invoquen la syscall. Añadimos la entrada correspondiente para `getancestor()`:
     ```c
     entry("getancestor");
     ```
6. **Modificar `yosoytupadre.c`:**
...
7. **Pruebas con valores de N`:**
    ...
    * **Probamos el caso donde retorne el -1 es decir con N=3:**
    ![Captura de pantalla (68)](https://github.com/Fredyxsen/xv6-riscvz/blob/Diego_Marin_T1/Captura%20de%20pantalla%20(68).png)

## Problemas encontrados:
  Falta de conocimiento en el ambiente trabajado:
    Más que nada en esta tarea el problema mas grande y unico que tuve para realizarla fue la falta de conocimiento y lo poco relacionado que estyabva con XV6, ya que despues de buscar bastante informacion en Youtube y complementarla con ayuda de la inteligencia artificial, pude entender y llegar a el proceso para lograr crear nuevas llamadas al sistema e implementarlas en mi XV6
